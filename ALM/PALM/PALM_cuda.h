...
public:
	...
	palmSsolver(int new_m, int new_n);
...
public:
	//      Allocates space to hold A with the correct storage on CPU and on GPU,
	//	and allocates space for other state vectors, etc... on the GPU.
	palmDsolver(int new_m, int new_n);
	...
	//        they will likely be copied and typeconverted internally.
	// 
	void solve(double *b, double *x, double *e, double tol = 1E-6, double tol_int = 1E-6, int maxIter = 50, int maxIter_alt = 50, int stoppingcriterion = 5, const double *xG = nullptr, double tol2 = 0.01);

public:

	double tau, tauInv;
	double *d_b, *d_xe, *d_old_xe, *d_A, *d_temp, *d_temp1, *d_temp2, *d_x, *d_e, *d_z, *d_G, *d_Gx, *d_Gz, *d_s, *d_Gx_old;
	double *d_lambda, *d_lambda_scaled, *d_x_old_apg, *d_xG;
	double * dual_temp;
	bool  *d_nz_x, *d_nz_x_prev;
	int block_size, grid_size;
	double nrm_b;
	...
	double dx, nxo, neo, de;
	int nIter_apg;
	double mu, t1, t2, muInv, muTauInv;
	bool converged_main, converged_apg;
	double eps;
