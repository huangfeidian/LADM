...
#include <chrono>
#include <stdio.h>
#include "../util/cache_align_storage.h"
namespace alsm
{
	...
	public:
		...
		cache_align_storage<std::atomic_bool>* my_turn;
		std::atomic_bool* work_finished;
		const std::chrono::nanoseconds wait_time;
		void recieve_sync()
		{
			while (my_turn->data.load()==false)
			{
				//std::this_thread::sleep_for(wait_time);
			}
			my_turn->data.store(false);
	...
	public:
		const int index;
		client(std::atomic_bool* in_work_finished, cache_align_storage<std::atomic_bool>* in_my_turn, std::atomic_int* in_free_thread_count, int in_wait_time, int in_index)
